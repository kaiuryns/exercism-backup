...
class EstateExecutor
  ...
  def self.assemble_account_number(secret_modifier)
    FAMILIES.sum do |family|
      family.bank_number_part(secret_modifier)
    end
  end

  def self.assemble_code
    red_total = FAMILIES.sum do |family|
      family::Red.code_fragment
    end

    blue_total = FAMILIES.sum do |family|
      family::Blue.code_fragment
    end
